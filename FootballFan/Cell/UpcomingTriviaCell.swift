...
import Foundation
import UIKit
class UpcomingTriviaCell: UITableViewCell {
        ...
        if timeEnd?.compare(timeNow) == ComparisonResult.orderedDescending {
            ...
            if(ispurchase){
                if(days == 0){
                    let remtime = String(format: "%02d:%02d:%02d", Int(hours as Double), Int(minites as Double), Int(scondes!))
                                           triviastatus?.text = "STARTS IN \(remtime)" //"\(Int(hours)) HOURS \(Int(minites)) MIN. \(Int(scondes as! Double)) SEC. TO PLAY"
                           }
                           else{
                    let dateFormatter = DateFormatter()
          ...
          //  triviastatus?.text = "LIVE NOW"
           let tabIndex:[String: Any] = ["index": currentIndex]
                       let notificationName = Notification.Name("trivialive")
