...
class FanUpdatesCell: UITableViewCell, ASAutoPlayVideoLayerContainer {
 ...
 @IBOutlet weak var coontentview: UIView!
    ...
    override func awakeFromNib() {
       ...
       // videoLayer.videoGravity = .resizeAspect
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(FanUpdatesCell.PlayAtIndex), name: notificationName, object: nil)
        ...
        if(appDelegate().isOnMyFanStories){
            ...
            if(curentlayer == inmycontacts){
               // print(curentlayer)
                progress?.isHidden = true
                print(inmycontacts)
                var dict1: [String: AnyObject] = appDelegate().arrFanUpdatesTeams[Int(inmycontacts)!] as! [String: AnyObject]
                dict1["isloader"] = false as AnyObject
                
                appDelegate().arrFanUpdatesTeams[Int(inmycontacts) ?? 0] = dict1 as AnyObject
            }
        }
        }
        */
    ...
    func appDelegate() -> AppDelegate {
        ...
        DispatchQueue.main.async{
            FanUpdatesCell.realDelegate = UIApplication.shared.delegate as? AppDelegate;
            dg.leave();
        }
        dg.wait();
        return FanUpdatesCell.realDelegate!;
    }
