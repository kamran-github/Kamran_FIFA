...
class CommentViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextViewDelegate{
 ...
 @IBOutlet weak var navItem: UINavigationItem!
                                                                                                                    ...
                                                                                                                    case .success(let value):
                                                                                                                        if let json = value as? [String: Any] {
                                                                                                                            ...
                                                                                                                            if(status1){
                                                                                                                                ...
                                                                                                                                if(self.appDelegate().arrFanUpdateComments.count>2){
    ...
    override func viewDidLoad() {
        ...
        self.appDelegate().arrFanUpdateComments = [AnyObject]()
        ...
        let notificationName1 = Notification.Name("_FanUpdateGetComments")
    ...
    @objc func FanUpdateGetComments(_ lastindex : Int)
    {
        ...
        } else
        {
          ...
          if ClassReachability.isConnectedToNetwork() {
        var dictRequest = [String: AnyObject]()
        dictRequest["cmd"] = "getupdatecomments" as AnyObject
        dictRequest["key"] = "kXfqS9wUug6gVKDB" as AnyObject
               dictRequest["device"] = "ios" as AnyObject
        do {
            //Creating Request Data
                                                                                                        ...
                                                                                                        if let json = value as? [String: Any] {
                                                                                                            ...
                                                                                                            if(status1){DispatchQueue.main.async {
                                                                                                               ...
                                                                                                               self.viewcount = json["viewcount"] as! Int64
                                                                                                                       self.appDelegate().updatecviewOnArrays(fanuid: self.fanupdateid,likecount: self.likecount,commentcount: self.commentcount, viewcount: self.viewcount)
                                                                                                               if(self.isFanPageCommentRefresh)
                                                                                                                                                              {
                                                                                                                                                               self.appDelegate().arrFanUpdateComments = response  as [AnyObject]
                                                                                                                                                              }
                                                                                                                                                              else
                                                                                                                                                              {
                                                                                                                                                               self.appDelegate().arrFanUpdateComments += response  as [AnyObject]
    ...
    @objc func refreshFanUpdateGetComments()
    {
        ...
        //TransperentLoadingIndicatorView.hide()
        closeRefresh()
        if(self.appDelegate().arrFanUpdateComments.count == 0){
    ...
    @objc func DeleteClick(_ longPressGestureRecognizer: UITapGestureRecognizer) {
        ...
        if let indexPath = storyTableView?.indexPathForRow(at: touchPoint) {
           ...
           /* let dict: NSDictionary? = appDelegate().arrFanUpdateComments[indexPath.row] as? NSDictionary
    ...
    func DeletePost(indx:IndexPath)  {
         if ClassReachability.isConnectedToNetwork() {
        let dict: NSDictionary? = appDelegate().arrFanUpdateComments[indx.row] as? NSDictionary
                                                                    ...
                                                                    switch response.result {
                                                                                                            case .success(let value):
                                                                                                                if let json = value as? [String: Any] {
                                                                                                                                                                                                let status1: Bool = json["success"] as! Bool
                                                                                                                    if(status1){DispatchQueue.main.async {
                                                                                                                        self.appDelegate().arrFanUpdateComments.remove(at: indx.row)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        print(appDelegate().arrFanUpdateComments.count)
        return self.appDelegate().arrFanUpdateComments.count
        
    }
    
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        //if(appDelegate().arrFanUpdateComments.count > 19)//
       // {
        
        let lastElement = appDelegate().arrFanUpdateComments.count - 1
        if(commentcount != 0){
        if(commentcount != appDelegate().arrFanUpdateComments.count){
        if indexPath.row == lastElement {
            // handle your logic here to get more items, add it to dataSource and reload tableview
            lastposition = appDelegate().arrFanUpdateComments.count //lastposition + 1
            FanUpdateGetComments(lastposition)
        }
    }
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let dic: NSDictionary? = self.appDelegate().arrFanUpdateComments[indexPath.row] as? NSDictionary
        //print(dic)
