...
class FanUpdatesListViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
 ...
 var tempFanUpdatesTeams: [AnyObject] = []
    ...
    override func viewDidLoad() {
       ...
       NotificationCenter.default.addObserver(self, selector: #selector(self.FanUpdateSaveLike), name: notificationName1, object: nil)
        ...
        let notifyresetStory = Notification.Name("resetStory")
        // Register to receive notification
        NotificationCenter.default.addObserver(self, selector: #selector(self.storyTablereload), name: notifyresetStory, object: nil)
        let storyUserblockFail = Notification.Name("storyUserblockFail")
                    
                    NotificationCenter.default.addObserver(self, selector: #selector(self.blockuserfail), name: storyUserblockFail, object: nil)
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.navigationItem.title = "Fan Stories"
       ...
       // let button2 = UIBarButtonItem(image: UIImage(named: "add_update"), style: .plain, target: self, action: #selector(FanUpdatesListViewController.AddPost(sender:)))
        //let rightSearchBarButto:UIBarButtonItem = button2
  ...
  @objc func storyTablereload()
                                                            ...
                                                            case .success(let value):
                                                                if let json = value as? [String: Any] {
                                                                    ...
                                                                    if(status1){DispatchQueue.main.async {
                                                                        ...
                                                                        if(self.appDelegate().isFanPageRefresh)
                                                                        {
                                                                           if(self.appDelegate().arrFanUpdatesTeams.count>2){
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return appDelegate().arrFanUpdatesTeams.count
    }
    ...
    @objc func MenuClick(_ longPressGestureRecognizer: UITapGestureRecognizer) {
          ...
          if(appDelegate().arrFanUpdatesTeams.count > 0){
              let dict: NSDictionary? = self.appDelegate().arrFanUpdatesTeams[longPressGestureRecognizer.view!.tag] as? NSDictionary
    ...
    @objc func DeleteClick(_ longPressGestureRecognizer: UITapGestureRecognizer) {
        ...
        if let indexPath = self.storyTableView?.indexPathForRow(at: touchPoint) {
            let dict: NSDictionary? = self.appDelegate().arrFanUpdatesTeams[indexPath.row] as? NSDictionary
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if(appDelegate().arrFanUpdatesTeams.count > 19)
        {
                                  ...
                                  // label.text = cell.ContentText!.text
                                   ...
                                   else
                                   {
                                      ...
                                      // print(label.frame.height)
                                       ...
                                       {
                                           let height = (label.frame.height) + 402.0
                                           //print("Height \((label.frame.height)).")
                                           //storyTableView?.rowHeight = CGFloat(height)
                                           return height
                                       }
                                       ...
                                       {
                                           let height = 402.0 //(cell.ContentText?.frame.height)! + 410.0
                                           //print("Height \((label.frame.height)).")
                                           return CGFloat(height)
                                           //storyTableView?.rowHeight = CGFloat(height)
                                       }
  ...
  /*  @objc func soundClick(_ longPressGestureRecognizer: UITapGestureRecognizer) {
   ...
   @IBAction func latestAction(){
    ...
    self.appDelegate().APIgetfanupdatestime = self.appDelegate().getUTCFormateDate()
    appDelegate().arrFanUpdatesTeams = tempFanUpdatesTeams
    storyTableView?.reloadData()
      }
