...
import AVKit
import Alamofire
class MediaDetailViewController:UIViewController,ASAutoPlayVideoLayerContainer {
    var videoURL: String?
 ...
 @IBOutlet weak var platformLogo: UIImageView!
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(MediaDetailViewController.PlayAtIndex), name: notificationName, object: nil)
        let notificationName_ffdeeplink = Notification.Name("_FetchedMediaByID")
        // Register to receive notification
        NotificationCenter.default.addObserver(self, selector: #selector(MediaDetailViewController.refreshView), name: notificationName_ffdeeplink, object: nil)
        let notificationName_updatecount = Notification.Name("_fanupdatecount")
               // Register to receive notification
               NotificationCenter.default.addObserver(self, selector: #selector(MediaDetailViewController.updatecount), name: notificationName_updatecount, object: nil)
        let notificationName_updatedelete = Notification.Name("_fanupdatedelete")
                      // Register to receive notification
                      NotificationCenter.default.addObserver(self, selector: #selector(MediaDetailViewController.updatedelete), name: notificationName_updatedelete, object: nil)
...
updateView()
    ...
    @objc func updateView()
    {
        ...
        if(newsdetail != nil)
        {
            ...
            var fanname = appDelegate().ExistingContact(username: (newsdetail.value(forKey: "username") as? String)!)
            ...
            let platform: String = newsdetail.value(forKey: "platform")as! String//
            if(platform == "YouTube"){
                 platformLogo.isHidden = false
                platformLogo.imageURL = newsdetail.value(forKey: "platformlogo") as? String
               
                if(newsdetail.value(forKey: "accountname") != nil)
                {
                let accountname = newsdetail.value(forKey: "accountname") as! String
                ...
                do {
                    ...
                    if(jsonDataMessage?.value(forKey: "subtype") != nil)
                    {
                        ...
                        if(subtype.contains("post")){
                           ...
                           // time?.isHidden = false
                            let longPressGesture_showchat:UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(showSource(_:)))
                            //longPressGesture.minimumPressDuration = 1.0 // 1 second press
                            longPressGesture_showchat.delegate = self as? UIGestureRecognizerDelegate
                            let longPressGesture_showchat1:UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(showSource(_:)))
                                //longPressGesture.minimumPressDuration = 1.0 // 1 second press
 ...
 /* if((newsdetail.value(forKey: "status") as? String) == "downloading")
                    ...
                    else
                    {
                        ...
                        if((label.frame.height) > 17)
                        {
                            var height = (label.frame.height) + 345.0
                        ...
                        {
                            var height = 345.0 //(cell.ContentText?.frame.height)! + 410.0
                            //print("Height \((label.frame.height)).")
                              if(platform == "YouTube"){
                                                          height = height - 20
                                                      }
            ...
            //Ravi Media
            if(newsdetail.value(forKey: "sourcelink") != nil)
            {
                let sourcelink:String = (newsdetail.value(forKey: "sourcelink") as? String)!
                if(!sourcelink.isEmpty)
                {
                    lblSource?.isHidden = true
                    btnSource?.isHidden = false
                    let source:String = (newsdetail.value(forKey: "source") as? String)!
                    btnSource.setTitle(source, for: UIControl.State.normal)
                    ...
                    username?.textColor = UIColor.init(hex: "007AFF")
                    if(platform == "YouTube"){
                                                                       lblSource?.isHidden = true
            ...
            likeText?.isUserInteractionEnabled = true
             let longPressGesture_source:UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(showSource(_:)))
                       //longPressGesture.minimumPressDuration = 1.0 // 1 second press
                       longPressGesture_source.delegate = self as? UIGestureRecognizerDelegate
                       
                       btnSource?.addGestureRecognizer(longPressGesture_source)
                       btnSource?.isUserInteractionEnabled = true
            ...
            let longPressGesture_platformLogo:UITapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(Showchat(_:)))
                                                  //longPressGesture.minimumPressDuration = 1.0 // 1 second press
                longPressGesture_platformLogo.delegate = self as? UIGestureRecognizerDelegate
                           platformLogo.addGestureRecognizer(longPressGesture_platformLogo)
                           platformLogo.isUserInteractionEnabled = true
            ...
            if(videolike == "inactive"){
                LCSview.isHidden = true
                LCcounttview.isHidden = true
    ...
    //Ravi Media
    @objc func showSource(_ longPressGestureRecognizer: UITapGestureRecognizer) {
        ...
        if ClassReachability.isConnectedToNetwork()
        {
            ...
            if(!sourceLink.isEmpty)
            {
               // UIApplication.shared.openURL(NSURL(string : sourceLink )! as URL)
                ...
                let messageContent = newsdetail.value(forKey: "message")as! String
                       ...
                       myTeamsController.stringurl = sourceLink
                                                                                                                                     ...
                                                                                                                                     case .success(let value):
                                                                                                                                         if let json = value as? [String: Any] {
                                                                                                                                             ...
                                                                                                                                             if(status1){DispatchQueue.main.async {
                                                                                                                                                 ...
                                                                                                                                                 self.appDelegate().mediaLikecount(fanuid: dict?.value(forKey: "id") as! Int64, likecount: json["likecount"] as! Int64, commentcount: json["commentcount"] as! Int64, viewcount: json["viewcount"] as! Int64, islike: dic.value(forKey: "liked") as! Bool)
                                                                                                                                                 }
    ...
    @objc func ShareClick(_ longPressGestureRecognizer: UITapGestureRecognizer) {
        //print("Share Click")
            do {
                ...
                let text = "\(decodedString1)\n\nVideo shared via Football Fan App.\n\nPlease follow the link:\n\(inviteurl)\n\nBy Fans for Fans where Fans have their voice heard."//title + "Fan Story shared via Football Fan App.\n\nPlease follow the link:\n"
