...
class TopFansViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    @IBOutlet weak var storyTableView: UITableView?
    let cellReuseIdentifier = "topfanscell"
    ...
    override func viewDidLoad() {
       ...
       // let settingsButton = UIBarButtonItem(image: settingsImage,  style: .plain, target: self, action: #selector(self.refreshView))
        let rightSearchBarButtonItem1:UIBarButtonItem = UIBarButtonItem(barButtonSystemItem: UIBarButtonItem.SystemItem.refresh, target: self, action: #selector(TopFansViewController.refreshView))
    ...
    @objc func refreshView() {
        ...
        if(appDelegate().isUserOnline)
        {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                ...
                self.getLeaderBoard(self.lastposition)
            }
        }
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
       ...
       // storyTableView?.reloadData()
        ...
        if(appDelegate().arrLeaderTopUsers.count <= 0)
        {
            
            notelable?.isHidden = false
            butcontacts?.isHidden = false
            storyTableView?.isHidden = true
            let bullet1 = "No Fans found in the Leader Board."
            let bullet2 = "You can be on the Leader Board by earning FanCoins. To learn more tap on “Earn more FanCoins?”."
    ...
    func appDelegate() -> AppDelegate {
        ...
        dg.enter()
        DispatchQueue.main.async{
            TopFansViewController.realDelegate = UIApplication.shared.delegate as? AppDelegate;
            dg.leave();
        }
        dg.wait();
        return TopFansViewController.realDelegate!;
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell: TopFansCell = tableView.dequeueReusableCell(withIdentifier: cellReuseIdentifier, for: indexPath) as! TopFansCell
        let dict: NSDictionary? = appDelegate().arrLeaderTopUsers[indexPath.row] as? NSDictionary
        if(dict != nil)
        {
            cell.rank?.text = "\(dict?.value(forKey: "rank") as! Int)"
            cell.fanCoins?.text = "\(self.appDelegate().formatNumber(dict?.value(forKey: "totalcoins") as? Int ?? 0))"
            let fanname = appDelegate().ExistingContact(username: (dict?.value(forKey: "username") as? String)!)
            cell.fanName?.text = fanname
            
            cell.fanImage?.image = UIImage(named: "avatar")
            if(dict?.value(forKey: "avatar") != nil)
            {
                ...
                if(!avatar.isEmpty)
                {
                    do {
                        ...
                        if fileManager.fileExists(atPath: getFilePath(url: avatar)) {
                            // print("file")
                            cell.fanImage?.image = UIImage(contentsOfFile: getFilePath(url: avatar))
                        } else {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return appDelegate().arrLeaderTopUsers.count
    }
