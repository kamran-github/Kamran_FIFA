...
class InAppPurchase : NSObject, SKProductsRequestDelegate, SKPaymentTransactionObserver {
	...
	let unlockTestInAppPurchase1ProductId = "test_fancoins"
	let unlockTestInAppPurchase2ProductId = "test_fancoins"
	...
	func productsRequest(_ request: SKProductsRequest, didReceive response: SKProductsResponse) {
		...
		if count > 0 {
			...
			print("No products")
            let notificationName = Notification.Name("_inappPurchasefail")
			...
			if let trans: SKPaymentTransaction = transaction as? SKPaymentTransaction {
				...
				case .purchased:
                   ...
                   /* print(trans.payment.productIdentifier)
                    print(trans.payment.applicationUsername)
                    print(trans.payment.quantity)
                    print(trans.payment.requestData)
                    ...
                    let tabIndex:[String: Any] = ["index": trans.transactionIdentifier as Any]
                    let notificationName = Notification.Name("_inappPurchaseSuccess")
			...
			let task = session.dataTask(with: storeRequest, completionHandler: { [weak self] (data, response, error) in
				...
				do {
					...
					/*if let date = self?.getExpirationDateFromResponse(jsonResponse as! NSDictionary) {
						print(date)
                        /*let tabIndex:[String: Any] = ["index": date]
	...
	func getExpirationDateFromResponse(_ jsonResponse: NSDictionary) -> AnyObject? {
        let receipt = jsonResponse["receipt"] as! NSDictionary
		if let receiptInfo: NSArray = receipt["in_app"] as? NSArray {
			...
			if let expiresDate = lastReceipt["transaction_id"] as? String {
                return expiresDate as AnyObject//formatter.date(from: expiresDate)
			}
